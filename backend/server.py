...
# Lithuania timezone - automatically handles EET/EEST (winter/summer time)
...
async def start_work_session(request: WorkSessionStart):
    ...
    logger.info(f"Started work session at GPS: {request.latitude}, {request.longitude}")
...
async def end_work_session(request: WorkSessionEnd):
    ...
    # Update session with end time and GPS
    ...
    logger.info(f"Ended work session at GPS: {request.latitude}, {request.longitude}, overtime: {overtime_minutes} min")
    
    # Get settings to check if auto-send email
    settings = await db.settings.find_one()
    auto_send = settings.get('auto_send_email_on_geofence', False) if settings else False
    
    # Auto-send email if enabled and there are overtime minutes
    if auto_send and overtime_minutes > 0:
        try:
            ...
            scheduled_start = updated_session.get('scheduled_start', 'N/A')
            
            # Format end time
            end_time_tz = end_time.astimezone(LITHUANIA_TZ)
            actual_end_str = end_time_tz.strftime('%H:%M')
            ...
            body_text = email_template.format(
                ...
                overtime_minutes=overtime_minutes,
                photo_count=len(updated_session.get('photos', []))
            )
...
{body_text}
            ...
            send_email_with_photos(recipient, subject, body_html, updated_session.get('photos', []))
            ...
            logger.info(f"Auto-sent overtime email for session {request.session_id}")
            
        except Exception as e:
            logger.error(f"Failed to auto-send email: {str(e)}")
            # Don't fail the session end if email fails
    
    updated_session = await db.sessions.find_one({"id": request.session_id})
    return {"success": True, "session": WorkSession(**updated_session).dict(), "email_sent": auto_send and overtime_minutes > 0}
...
async def send_overtime_email(request: EmailSendRequest):
    ...
    # Get actual end time and convert to Lithuania timezone
    end_time = session.get('end_time')
    if end_time:
        if end_time.tzinfo is None:
            # If naive, assume UTC
            end_time = end_time.replace(tzinfo=timezone.utc)
        # Convert to Lithuania timezone
        end_time = end_time.astimezone(LITHUANIA_TZ)
    ...
    body_text = email_template.format(
        date=session['date'],
        start_time=scheduled_start,  # Use scheduled start from work schedule
        end_time=actual_end_str,  # Use actual end time with Lithuania timezone
        overtime_hours=f"{overtime_hours:.2f}",
        overtime_minutes=overtime_minutes,
...
async def shutdown_db_client():
    client.close()# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s'
)
